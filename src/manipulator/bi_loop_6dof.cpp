#include "manipulator_6dof.h"           // 6축 모델 헤더로 변경
#include "collision_checker_3d_6dof.h"  // 사용자가 지정한 6축용 충돌 체커 헤더
#include "bi_mppi_6dof.h"               // 6축 MPPI 코어 헤더로 변경
...
int main() {
    // 1. 6자유도 매니퓰레이터 모델 생성 (최대 조인트 속도 2.0 rad/s)
    ...
    // 각 조인트(6개)별 노이즈 분산 세팅 (대각 행렬, 6차원)
